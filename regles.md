...
Si le taux de boisson dépace la limite max, le joueur passe son tour (voir: PLS involontaire).

Pour la suite du tour, tous les jetons de boisson gagné sont placé en attente. Ils seront ajouté au début du prochain tour du joueur.
...
Quand le taux de boisson au dessus du max.
...
Au tour suivant on passe à l'action chopper si ca a réussi.
...
Et heu? Est-ce qu'ils ont été discrets ?

Pour cela lancez un dé7 et vous divisé la valeur par 2. (Si il s'agit de 2 perso (pas 1 perso et 1 PNJ) un seul lancé de dé pour les 2.
